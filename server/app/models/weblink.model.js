...
/* backup ....................*/
	...
	/* search link from database .................................*/	
	function searchWeblink(searchString) {
		var deferred = q.defer();
		console.log(searchString);
		Weblink.find({$text: {$search: searchString}},{ score : { $meta: "textScore" } })
			.sort({ score : { $meta : 'textScore' } }).populate('createdBy').exec(function(err, item){
			if(err) {
				deferred.reject(err)
			}  
				console.log('success');
	...
	// for mongodb 2.4 version which dont support text search
/*	function searchWeblink(searchString) {
		var deferred = q.defer();
			console.log('PODUC');
			Weblink.find({title:searchString}, function(err, result){
				if(err) {
					deferred.reject(err)
				} 
				console.log(result); 
