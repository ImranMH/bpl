...
module.exports = function(passport){
  ...
  function register(req, res){
     ...
     res.json(req.user)
      // User.registerUser(newUser).then((user)=>{
    ...
    // if(req.user){
    //   res.json({user:req.user, status:false, msg: "you already a register"})
    // } else {
    //   let newUser = req.body
    //   User.registerUser(newUser).then((user)=>{
    //     res.json({msg: "registration Unsuccessful",user:user})
    //   })
    // } 
  }
  ...
  function searchUser(req, res){
    console.log('search from user controller');
      ...
      User.findUserByProfile(sessionUser._id).then(function(user){
        User.getTopFollowerData(user._id).then((data) => {
          ...
          User.getTopFollowingData(user._id).then((following)=>{
            ...
            status.session= req.session;
            status.login= true;
            status.sessionuser= sessionUser;
